...
class Challenge {
  ...
  constructor(input: string) {
    ...
    this.almanac = {
      seeds: sections[0].split(' ').slice(1).map((str) => +str),
  ...
  solve = (extra: boolean = false): number => {
  ...
  private parseMap = (descriptor: string, sections: Array<string>): Array<AlmanacMap> => {
    ...
    const section = sections.find((sec) => sec.startsWith(descriptor))?.split('\n').slice(1);
  ...
  private getSeedRanges = (seedInput: string): Array<SeedRange> => {
    ...
    const parsedSeedInput: Array<number> = seedInput.split(' ').slice(1).map(str => +str);
